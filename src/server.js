...
import { etsyRequest, exchangeCode, getOwnShop, getOwnShopId } from "./etsy.js";
...
async function route(request, response) {
  ...
  if (request.method === "GET" && url.pathname === "/api/me/shop") {
    return send(response, 200, await getOwnShop(config));
  }
  if (request.method === "GET" && url.pathname === "/api/me/sales") {
    ...
    return send(response, 200, await etsyRequest(config, `/v3/application/shops/${await getOwnShopId(config)}/receipts`, { query, authenticated: true }));
  }
  if (request.method === "GET" && url.pathname === "/api/me/listings") {
    const query = { ...pageQuery(url), state: url.searchParams.get("state") || "active" };
    return send(response, 200, await etsyRequest(config, `/v3/application/shops/${await getOwnShopId(config)}/listings`, { query, authenticated: true }));
  }
  if (request.method === "POST" && url.pathname === "/api/me/listings") {
    return send(response, 201, await etsyRequest(config, `/v3/application/shops/${await getOwnShopId(config)}/listings`, { method: "POST", body: await jsonBody(request), authenticated: true }));
  }
  if (request.method === "PATCH" && parts.length === 4 && parts.slice(0, 3).join("/") === "api/me/listings" && /^\d+$/.test(parts[3])) {
    return send(response, 200, await etsyRequest(config, `/v3/application/shops/${await getOwnShopId(config)}/listings/${parts[3]}`, { method: "PATCH", body: await jsonBody(request), authenticated: true }));
  }
